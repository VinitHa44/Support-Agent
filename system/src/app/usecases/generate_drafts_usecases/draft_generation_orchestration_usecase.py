...
from system.src.app.usecases.generate_drafts_usecases.request_logging_usecase import (
    RequestLoggingUsecase,
)
from system.src.app.usecases.generate_drafts_usecases.template_storage_usecase import (
...
class DraftGenerationOrchestrationUsecase:
    ...
    async def execute_draft_generation_workflow(
        ...
        if categories and len(categories) > 0:
            ...
            tasks.append(asyncio.sleep(0, result=[]))
            logging.debug(
                "Skipping dataset search - empty or missing categories"
            )
        ...
        if len(generated_drafts.get("drafts", [])) > 1:
            ...
            logging.debug(
                "Single draft generated, returning directly without review"
            )
        ...
        await self.request_logging_usecase.log_request(
            ...
            rocket_docs_response,
            dataset_response,
            multiple_drafts_generated=len(generated_drafts.get("drafts", []))
            > 1,
    ...
    async def _handle_review_process(
        ...
        try:
            ...
            response, status = (
                await self.websocket_manager.wait_for_draft_response(user_id)
            )
            ...
            if status == "success" and response:
                ...
                if is_skip:
                    logging.debug(
                        "User cancelled draft review - using first draft as fallback"
                    )
