...
class DataInsertUsecaseHelper:
    ...
    async def generate_embeddings(self, examples: List[Dict]) -> List[Dict]:
        ...
        for i in range(0, len(examples), batch_size):
            ...
            texts = [f"Subject: {example['subject']}\n{example['query']}" for example in batch]
            
            try:
                dense_embeddings = await self.embedding_service.pinecone_dense_embeddings(texts)
                sparse_embeddings = self.embedding_service.pinecone_sparse_embeddings(texts)
                ...
                loggers["main"].error(f"Error generating embeddings for batch: {str(e)}")
                continue
    ...
    async def upsert_chunks_in_pinecone(self, chunks: List[Dict]) -> Dict:
        ...
        if vectors_to_upsert:
            result = await self.pinecone_service.upsert_vectors_simplified(vectors_to_upsert)
            loggers["main"].info(f"Upserted {len(vectors_to_upsert)} vectors to Pinecone")
    ...
    async def ensure_pinecone_index_exists(self):
        ...
        try:
            ...
            indexes_response = await self.pinecone_service.list_pinecone_indexes()
            existing_indexes = indexes_response.get("indexes", [])
            index_names = [idx.get("name") for idx in existing_indexes]
            
            loggers["main"].info(f"Found existing Pinecone indexes: {index_names}")
            
            # Check if our target index exists
            if settings.PINECONE_INDEX_NAME not in index_names:
                loggers["main"].warning(f"Index '{settings.PINECONE_INDEX_NAME}' not found. Creating it...")
                ...
                loggers["main"].info(f"Successfully created Pinecone index: {settings.PINECONE_INDEX_NAME}")
            else:
                loggers["main"].info(f"Pinecone index '{settings.PINECONE_INDEX_NAME}' already exists")
                
        except Exception as e:
            loggers["main"].error(f"Error ensuring Pinecone index exists: {str(e)}")
            raise HTTPException(
                status_code=500,
