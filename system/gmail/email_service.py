...
class EmailService:
    def __init__(self):
        self.auth = GmailAuth()
        self.service = None
        self.current_message_id = None  # For tracking during attachment processing
        self.is_first_poll_after_startup = True  # Flag to track if this is first poll since startup
    ...
    def get_new_emails(self) -> List[Dict]:
        ...
        try:
            ...
            if self.is_first_poll_after_startup:
                logger.info("STARTUP: Mandatory fetch of most recent unread email (ignoring stored history)")
                ...
                logger.info(f"STARTUP: Complete. Set history ID to {current_history_id} for future polls")
                ...
                if last_history_id:
                    ...
                    logger.warning("No history ID found during normal polling - getting recent unread")
                    new_emails = self._get_recent_unread_emails()
        ...
        try:
            ...
            if messages:
                ...
                if email_data:
                    ...
                    logger.info(f"Found most recent unread email from {email_data['sender']} - Subject: {email_data['subject']}")
                else:
                    logger.info("Most recent email was filtered out (likely not INBOX+UNREAD)")
            else:
    ...
    def _get_email_details(self, message_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if 'INBOX' not in labels or 'UNREAD' not in labels:
                logger.info(f"Skipping email {message_id} - Labels: {labels} (not INBOX+UNREAD)")
                return None
            
            logger.info(f"Processing email {message_id} - Valid INBOX+UNREAD labels")
            ...
            body, attachments = self._extract_content_and_attachments(message['payload'])
            ...
            email_data = {
                ...
                'has_images': len([att for att in attachments if att['is_image']]) > 0,
                'is_unread': True,
        ...
        def process_part(part):
            ...
            # Handle attachments (including images)
                ...
                if attachment_info['is_image']:
                    try:
                        image_data = self.get_attachment_data(self.current_message_id, part['body']['attachmentId'])
                        if image_data:
                            attachment_info['base64_data'] = base64.b64encode(image_data).decode('utf-8')
                            attachment_info['data_uri'] = f"data:{mime_type};base64,{attachment_info['base64_data']}"
                            logger.info(f"Added base64 data for image: {filename}")
                        else:
                            attachment_info['base64_data'] = None
                            attachment_info['data_uri'] = None
                            logger.warning(f"Failed to download image data for: {filename}")
                    except Exception as e:
    ...
    def create_draft(self, to_email: str, subject: str, body: str, in_reply_to: str = None, thread_id: str = None) -> bool:
        ...
        try:
            ...
            message = {
                'raw': self._create_message_raw(to_email, subject, body, in_reply_to)
            }
    ...
    def _create_message_raw(self, to_email: str, subject: str, body: str, in_reply_to: str = None) -> str:
